...
class CurationPublicIntegrationTest {
    ...
    @Test
    void 실제_HTTP로_공개_큐레이션_목록을_조회한다() throws Exception {
        HttpRequest request = HttpRequest.newBuilder(
            URI.create("http://localhost:" + serverPort + "/api/curations?page=1&size=9")
        ).GET().build();
    ...
    void 인증_없이_공개_큐레이션만_목록으로_조회한다() throws Exception {
        ...
        mockMvc.perform(get("/api/curations").contextPath("/api"))
    ...
    void 인증_없이_공개_큐레이션_상세를_조회한다() throws Exception {
        ...
        mockMvc.perform(get("/api/curations/{curationId}", curation.getId()).contextPath("/api"))
            .andExpect(status().isOk())
    ...
    private User saveLibrarian(String email) {
        return userRepository.saveAndFlush(new User(
            UserRole.LIBRARIAN, "김도서", email, "encoded-password", "책지기",
            "부산광역시 금정도서관", "121018", "자료운영팀"
        ));
    }
}
