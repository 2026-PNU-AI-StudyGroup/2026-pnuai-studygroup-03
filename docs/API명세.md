...
### 2026-08-20 — fix 브랜치 안정성 개선 통합

- 공개 큐레이션 목록·상세 조회 API를 추가했습니다.
- 새 큐레이션은 `isPublic=true`를 명시한 경우에만 공개되며, 수정 시 `isPublic`을 생략하면 기존 상태를 유지합니다.
- 후보군 작업 조회는 작업 요청자에게만 허용합니다.
- 비동기 작업 대기열이 가득 찬 경우 `503 JOB_005`를 반환하고 접수된 작업을 실패 상태로 전환합니다.
- 도서관별 DB 잠금으로 같은 도서관의 동시 후보군 생성 요청을 직렬화합니다.

### 2026-08-19 — 카카오 우선 상세 조회·KDC 균형 200권

도서 상세 공급자 순서를 **카카오 ISBN 검색 → 알라딘 → 정보나루 최후 폴백**으로 변경했습니다.
카카오나 알라딘에서 제목·저자·출판사·소개글을 확보하면 정보나루 `srchDtlList`를 호출하지 않습니다.
따라서 작은 정보나루 일일 호출량은 인기·장서·대출 순위처럼 정보나루에서만 얻을 수 있는 데이터에 남겨 둡니다.

잠자는 도서 기본 후보군은 60권에서 **200권**으로 확대했습니다. CSV의 `주제분류번호`를 후보 변환 과정에서
유실하던 문제를 고쳤고, KDC 0~9 대분류를 우선 20권씩 채운 뒤 부족한 분야의 몫만 다른 분야로 보충합니다.
`GET /libraries/{libraryCode}/hidden-books` 응답에는 `kdcCode`가 추가됩니다.

> 아래 2026-08-15의 알라딘 우선 결정은 당시 변경 이력이며, 현재 동작은 이 절의 카카오 우선순위가 최종입니다.
...
- **4.2 `POST /recommendations`**: 요청에 `limit`(선택, 기본·최대 15) 추가. 도서관의 잠자는 책 200권은 유지하면서 서버가 관련도 상위 60권을 사전 선별하고, AI는 최종 최대 15권만 생성합니다. 동일 조건 결과는 15분간 캐싱합니다. 응답에 `libraryName`, `callNumber`, `shelfName` 추가.
- **4.3 `POST /recommendations/compare`**: 응답을 24시간 캐싱합니다.
- **4.4 `POST /recommendations/explore`**: 응답에 `libraryName`, `callNumber`, `shelfName` 추가. 200권 후보군에서 서버가 최대 50권을 사전 선별하고 AI가 최종 최대 9권만 반환하며, 동일 조건은 15분 캐싱합니다.
...
저장된 후보군을 그대로 반환합니다. **AI나 외부 도서 API를 호출하지 않습니다.** 정보 품질이 좋은 순으로 정렬합니다.
`reason`은 그 도서가 오늘의 책·랜덤으로 노출될 때 생성되므로 아직 `null`일 수 있고, 이때는 `description`을 쓰면 됩니다.
...
```json
{
  ...
  "data": {
    "content": [{
      ...
      "shelfName": "[강서구]종합자료실", "kdcCode": "8", "source": "DEMO_SEED",
      "keywords": ["문학", "한국문학", "소설"]
    }],
...
| limit | AI가 최종 선정할 추천 수 **(선택)**. 기본값과 최댓값은 15입니다. 프론트는 15권을 한 번에 받은 뒤 처음 6권과 `추천 더 보기` 9권으로 나눠 표시합니다. |

성능과 안정성을 위해 DB의 도서관별 잠자는 책 후보군 최대 200권을 그대로 AI에 보내지 않습니다. 백엔드가 제목·키워드·소개로 서버 관련성을 계산해 관련 후보를 우선 최대 60권까지 사전 선별하며, 이 단계에는 외부 API나 AI 호출이 없습니다. 단어가 정확히 일치하는 후보가 20권보다 적으면 독자층이 맞는 상위 후보를 최대 20권까지 보충해 AI가 의미 관련성을 판단하게 합니다. AI에는 소개를 권당 최대 400자로 제한해 전달하고 최종 추천 최대 15권만 요청합니다. 기준 인기 도서와 ISBN이 같은 도서는 후보에서 제외됩니다. 실시간 트렌드 추천용 200권 후보군과 KDC 분야 균형에는 영향을 주지 않습니다.

기준 인기 도서의 제목·저자·소개로 예상 독자층(`CHILD`, `TEEN`, `ADULT`, `GENERAL`)을 판정합니다. 후보 도서의 `shelfName`과 소개에 `어린이`, `아동`, `유아`, `초등`, `학습만화`, `그림책`, `새싹`, `만화코너` 등의 명확한 표지가 있으면 어린이 도서로 판정하며, 성인 기준 도서 추천에서는 AI 호출 전에 제외합니다. 프롬프트에도 기준 도서 정보, 후보 독자층, 서가명, 서버 관련성 점수를 전달합니다. 서버 단어 관련성이 낮은 후보는 AI 키워드 관련성이 70점 이상일 때만 최종 통과하므로 의미상 관련된 책은 살리면서 낮은 확신의 무관한 추천은 차단합니다. 적합한 후보가 없으면 빈 배열을 반환합니다.
...
도서관의 잠자는 책 최대 200권은 그대로 유지합니다. 백엔드는 기준 도서와 후보 도서의 제목·키워드·소개 겹침, `EASIER`·`DEEPER`의 경우 소개 길이 차이까지 반영해 최대 50권을 사전 선별합니다. 기준 도서와 독자층이 맞지 않는 어린이·청소년 후보는 AI 호출 전에 제외하며, 소개는 권당 최대 400자로 제한합니다. AI는 50권 전체의 점수를 생성하지 않고 최종 최대 9권만 반환합니다. 동일한 `isbn + libraryCode + type` 결과는 15분간 캐싱합니다.
...
- `GET /curations?page=1&size=9`: 공개 큐레이션을 최신순으로 조회합니다. `size`는 최대 30입니다.
- `GET /curations/{curationId}`: 공개 큐레이션의 도서 구성과 사서 코멘트를 조회합니다.

두 API 모두 인증 없이 호출할 수 있습니다. `isPublic=true`인 큐레이션만 반환하며, 비공개 또는 존재하지 않는 ID는 동일하게 `404 CURATION_001`을 반환합니다.
...
#### 실제 HTTP 검증 결과 (2026-08-20)

현재 소스를 H2 테스트 DB와 랜덤 포트로 실행한 뒤 실제 HTTP 요청으로 공개 목록을 확인했다.
...
통합 테스트는 공개 목록·상세의 `200` 응답과 비공개 상세의 `404 CURATION_001`을 함께 검증한다. 실제 외부 API와 통합 환경의 상세 결과는 [통합 및 실제 API 검증 결과](./통합및실API검증결과.md)를 참고한다.
...
> **처리 시간 주의:** 후보 도서마다 카카오 ISBN 상세를 조회하고, 부족한 경우에만 알라딘·정보나루로 폴백하므로 산출에 시간이 걸릴 수 있습니다.
> 기본 후보군은 200권이며 KDC 0~9를 우선 20권씩 채우고, 특정 분야 장서가 부족하면 남는 분야에서 보충합니다.
